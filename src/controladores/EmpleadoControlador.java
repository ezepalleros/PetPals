...
public class EmpleadoControlador implements EmpleadoRepository {
	...
	public List<Empleado> getAllEmployees() {
		...
		try {
			...
			while (resultSet.next()) {
				Empleado empleado = new Empleado(resultSet.getInt("codEmp"), resultSet.getString("nomEmp"),
						resultSet.getString("mailEmp"), resultSet.getString("telEmp"),
						resultSet.getDate("antiEmp").toLocalDate(), resultSet.getString("detallEmp"),
						resultSet.getInt("califEmp"));
	...
	public Empleado getEmployeeById(int id) {
		...
		try {
			...
			if (resultSet.next()) {
				empleados = new Empleado(resultSet.getInt("codEmp"), resultSet.getString("nomEmp"),
						resultSet.getString("mailEmp"), resultSet.getString("telEmp"),
						resultSet.getDate("antiEmp").toLocalDate(), resultSet.getString("detallEmp"),
						resultSet.getInt("califEmp"));
			}
	...
	public void addEmployee(Empleado empleado) {
		try {
			PreparedStatement statement = connection.prepareStatement(
					"INSERT INTO empleado (nomEmp, mailEmp, telEmp, antiEmp, detallEmp, califEmp) VALUES (?, ?, ?, ?, ?, ?)");
	...
	public void updateEmployee(Empleado empleado) {
		try {
			PreparedStatement statement = connection.prepareStatement(
					"UPDATE empleado SET nomEmp = ?, mailEmp = ?, telEmp = ?, antiEmp = ?, detallEmp = ?, califEmp = ? WHERE codEmp = ?");
			statement.setString(1, empleado.getNomUsu());
			statement.setString(2, empleado.getMailUsu());
			statement.setString(3, empleado.getTelUsu());
			statement.setDate(4, Date.valueOf(empleado.getAntiEmp()));
