...
public class GerenteControlador implements GerenteRepository {
    ...
    public List<Gerente> getAllManagers() {
        ...
        try {
            ...
            while (resultSet.next()) {
            	Gerente gerente = new Gerente(resultSet.getInt("codGer"), resultSet.getString("nomGer"), resultSet.getString("mailGer"), resultSet.getString("numGer"), resultSet.getInt("salGer"));
    ...
    public Gerente getManagerById(int id) {
        ...
        try {
            ...
            if (resultSet.next()) {
            	gerentes = new Gerente(resultSet.getInt("codGer"), resultSet.getString("nomGer"), resultSet.getString("mailGer"), resultSet.getString("numGer"), resultSet.getInt("salGer"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void addManager(Gerente gerente) {
        try {
            PreparedStatement statement = connection.prepareStatement("INSERT INTO gerente (nomGer, mailGer, numGer, salGer) VALUES (?, ?, ?, ?)");
    ...
    public void updateManager(Gerente gerente) {
        try {
            PreparedStatement statement = connection.prepareStatement("UPDATE gerente SET nomGer = ?, mailGer = ?, numGer = ?, salGer = ? WHERE codGer = ?");
            statement.setString(1, gerente.getNomUsu());
            statement.setString(2, gerente.getMailUsu());
            statement.setString(3, gerente.getTelUsu());
            statement.setInt(4, gerente.getSalGer());
