	...
	public PantallaReaAdo(int codCli) {
		...
		btnSolicitarAdopcion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (seleccionada != null && seleccionada.getAdoptar() == 1) {
					...
					Adoptar nuevaAdopcion = new Adoptar(0, seleccionada.getCodMas(), seleccionada.getVariMas(),
							seleccionada.getEdadMas(), codCli);
		...
		selectionModel.addListSelectionListener(e -> {
			if (!e.getValueIsAdjusting()) {
				...
				if (selectedRow != -1) {
					seleccionada = new Mascota((int) table.getValueAt(selectedRow, 0),
							(String) table.getValueAt(selectedRow, 1), (String) table.getValueAt(selectedRow, 2),
							(String) table.getValueAt(selectedRow, 3), (int) table.getValueAt(selectedRow, 4),
							(int) table.getValueAt(selectedRow, 5), (String) table.getValueAt(selectedRow, 6),
							(int) table.getValueAt(selectedRow, 7), (int) table.getValueAt(selectedRow, 8),
							(int) table.getValueAt(selectedRow, 9), (int) table.getValueAt(selectedRow, 10));
					elemento.setText(
							"Seleccionado: ID=" + seleccionada.getCodMas() + ", Nombre=" + seleccionada.getNomMas());
				} else {
					seleccionada = null;
