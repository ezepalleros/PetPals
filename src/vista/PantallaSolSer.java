...
public class PantallaSolSer extends JFrame {
	...
	public PantallaSolSer(List<Mascota> mascotas, int codigoCliente) {
		...
		String[] columnNames = { "ID", "Nombre", "Día", "Hora Inicio", "Hora Fin", "Perro", "Gato", "Ave", "Roedor",
				"Reptil", "PrecioPerro", "PrecioGato", "PrecioAve", "PrecioRoedor", "PrecioReptil" };
		...
		comboBoxMascota.setRenderer(new javax.swing.ListCellRenderer<Mascota>() {
			@Override
			public java.awt.Component getListCellRendererComponent(javax.swing.JList<? extends Mascota> list,
					Mascota value, int index, boolean isSelected, boolean cellHasFocus) {
			...
			if (!e.getValueIsAdjusting()) {
				...
				if (selectedRow != -1) {
					...
					try {
						...
						seleccionado = new Servicio((int) rowData[0], (String) rowData[1], (LocalDate) rowData[2],
								(LocalDateTime) rowData[3], (LocalDateTime) rowData[4],
								((Integer) rowData[5]).intValue(), ((Integer) rowData[6]).intValue(),
								((Integer) rowData[7]).intValue(), ((Integer) rowData[8]).intValue(),
								((Integer) rowData[9]).intValue(), perro, gato, ave, roedor, reptil);
					} catch (NullPointerException | ClassCastException ex) {
						ex.printStackTrace();
						seleccionado = null;
					}
					lblSeleccionado.setText(
							"Seleccionado: ID=" + seleccionado.getCodSer() + ", Nombre=" + seleccionado.getNomSer());
				} else {
					seleccionado = null;
	...
	private void validarYCrearSolicitud() {
		...
		try {
			...
			if (horaInicio.isBefore(LocalTime.of(10, 0)) || horaFin.isAfter(LocalTime.of(20, 30))
		...
		if (!error) {
			Solicitud solicitud = new Solicitud(0, seleccionado.getCodSer(), fecha, LocalDateTime.of(fecha, horaInicio),
					LocalDateTime.of(fecha, horaFin), mascota.getCodMas(), mascota.getTipoMas(), mascota.getCaracMas());
			SolicitudControlador solicitudControlador = new SolicitudControlador();
			solicitudControlador.addRequest(solicitud);
			JOptionPane.showMessageDialog(this, "Solicitud creada exitosamente.", "Éxito",
